                    ...
                    <ListItem disablePadding>
                      <ListItemButton
                        sx={{
                          ...
                          gap: 1.5,
                          borderRadius: '5px',
                          bgcolor: selectedBuilding?.name === b.name ? '#e3f2fd' : 'transparent',
                          '&:hover': {
                            bgcolor: selectedBuilding?.name === b.name ? '#e3f2fd' : '#f5f5f5'
                          }
                        }}
                        onClick={() => handleBuildingClick(b)}
                      >
                        <ExpandMoreIcon sx={{ fontSize: 20, transform: expandedBuilding === b.name ? 'rotate(0deg)' : 'rotate(-90deg)', transition: 'transform 0.3s' }} />
                        <ListItemText primary={b.name} primaryTypographyProps={{ variant: 'body2', fontWeight: selectedBuilding?.name === b.name ? 600 : 400 }} />
                      </ListItemButton>
                    </ListItem>
                    {expandedBuilding === b.name && (
                      <List dense sx={{ pl: 2, py: 0, pb: 2 }}>
                        {buildingMenuItems.map((item) => {
                          ...
                          return (
                            <ListItem key={item.label} disablePadding>
                              ...
                              >
                                <Box sx={{ width: 28, height: 28, bgcolor: isActive ? '#1976d2' : '#f0f0f0', borderRadius: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background-color 0.2s' }}>
                                  <item.icon sx={{ fontSize: 16, color: isActive ? '#fff' : 'inherit', transition: 'color 0.2s' }} />
                                </Box>
                                <ListItemText primary={item.label} primaryTypographyProps={{ variant: 'body2', fontWeight: isActive ? 600 : 400 }} />
                              </ListItemButton>
                            </ListItem>
                          );
                        })}

...
export default function ComfortPerformancePage({ themeScore = 92, themeTrend = 5, onNavigateToDashboard, onBuildingSelect, onViewAllBuildings }: ComfortPerformancePageProps) {
  ...
  const menuItems: { key: ViewMode; label: string; color: string; icon: React.ReactNode }[] = [
    { key: 'theme', label: 'Comfort KPI', color: colors.brand, icon: <SpaOutlinedIcon sx={{ fontSize: 16 }} /> },
    { key: 'temperature', label: 'Temperature', color: topics[0].chartColor, icon: <ThermostatOutlinedIcon sx={{ fontSize: 16 }} /> },
    { key: 'humidity', label: 'Relative Humidity', color: topics[1].chartColor, icon: <WaterDropOutlinedIcon sx={{ fontSize: 16 }} /> },
    { key: 'air_quality', label: 'Air Quality', color: topics[2].chartColor, icon: <AirOutlinedIcon sx={{ fontSize: 16 }} /> },
  ];
              ...
              return (
                <Box
                  ...
                  sx={{
                    ...
                    bgcolor: isActive ? `${item.color}14` : 'transparent',
                    transition: 'all 0.15s ease',
                    '&:hover': { bgcolor: isActive ? `${item.color}20` : 'action.hover' },
                  }}
                >
                  <Box sx={{
                    display: 'flex',

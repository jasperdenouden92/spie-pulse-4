...
export default function YearRangeSlider({
  fromYear, toYear, onChange, minYear, maxYear,
  ...
  const toIdx = Math.max(fromIdx, years.indexOf(toYear));
  ...
  useEffect(() => {
    ...
    const handleUp = () => {
      const current = dragRef.current;
      if (!current) return;
      onChange(years[current.startIdx], years[current.endIdx]);
      setDrag(null);
    };
  ...
  }, [drag, idxFromClientX, onChange, years]);
        ...
        return (
          <Box
            ...
            sx={{
              ...
              cursor: 'pointer',
              '&:hover .yr-tick-dot': { transform: 'translateX(-50%) scale(1.4)' },
            }}
          >
            <Typography sx={{
